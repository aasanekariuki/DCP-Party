...
export function PolicyPreview() {
  const reduceMotion = useReducedMotion();
  const policies = getPolicies().slice(0, 6);
  ...
  return (
    <Section decorated animateEntry={false} className="relative">
      {/* Background ambient orbs in Jungle & Mint Green */}
      ...
      {/* Ambient floating green particles */}
      <FloatingParticle color="jungle" className="left-[5%] top-[25%] h-2.5 w-2.5" delay={0.2} duration={5.5} />
      ...
      <div className="relative z-10 mb-10 flex flex-wrap items-end justify-between gap-6 sm:mb-12">
        <div className="max-w-xl">
          ...
          <p className="mt-2 text-sm text-[var(--color-ink-soft,#64748B)] sm:text-base">
            Structured legislative proposals and verified civic commitments designed for long-term governance.
          </p>
        </div>
        ...
        >
          <Button href="/policies" variant="secondary" className="group inline-flex items-center gap-2 shadow-2xs">
            <span>Browse all priorities</span>
            <Compass className="h-4 w-4 text-emerald-600 transition-transform duration-300 group-hover:rotate-45" />
          </Button>
        </motion.div>
      </div>

      {/* Policy grid using Jungle Green variables instead of Slate/Blue */}
      <motion.div
      ...
      >
        {policies.map((policy, index) => (
          <motion.div key={policy.id || policy.slug} variants={reduceMotion ? undefined : cardVariants}>
            <Link
